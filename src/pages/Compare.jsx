...
export default function Compare() {
    ...
    async function loadExperiments() {
      try {
        ...
        if (data && Array.isArray(data.results)) {
          ...
          console.warn(
            "Experiments data is not an array, using empty array instead"
          );
          setExperiments([]);
        }
      } catch (err) {
  ...
  // Safely get selected runs
  const runA = selected[0]
    ? experiments.find((e) => e.id === selected[0]) || null
    : null;
  const runB = selected[1]
    ? experiments.find((e) => e.id === selected[1]) || null
    : null;
          ...
          {experiments.map((exp) => (
            ...
            >
              <input
                ...
                disabled={!selected.includes(exp.id) && selected.length === 2}
            ...
            <div className="mt-10">
              ...
              <table className="min-w-full bg-gray-900 rounded-xl mb-10">
                ...
                <tbody>
                  {metricRow(
                    "Accuracy",
                    runA.metrics?.accuracy,
                    runB.metrics?.accuracy
                  )}
                  {metricRow(
                    "Precision",
                    runA.metrics?.precision,
                    runB.metrics?.precision
                  )}
                  {metricRow(
                    "Recall",
                    runA.metrics?.recall,
...
function metricRow(label, a, b) {
  return (
    <tr className="border-b border-gray-800" key={label}>
      ...
      <td className="p-3">{a ?? "—"}</td> <td className="p-3">{b ?? "—"}</td>{" "}
    </tr>
  );
}
