...
def extract_code_from_notebook(notebook_path, output_path):
    ...
    "DATASETS_DIR = Path(__file__).resolve().parent.parent.parent / 'datasets'\n"
        ...
        "\n"
        "    # Helper: attempt to save a classification_report JSON to the backend\n"
        "    def save_confusion_to_db(report, model_name='ConvertedNotebook', params=None, duration_s=None, backend_url='http://localhost:5000'):\n"
        "        \"\"\"Post a `classification_report`-style dict to the backend\n"
        "        as an experiment so the UI can display the confusion matrix.\n"
        "        If `requests` is unavailable the helper will silently fail.\n"
        "        \"\"\"\n"
        "        try:\n"
        "            import json\n"
        "            try:\n"
        ...
        "                'params': params or {},\n"
        "                # the API expects `results` to contain raw metrics (we store the report there)\n"
        "                'results': {'classification_report': report},\n"
        "                'duration_s': duration_s,\n"
        "            }\n"
        "            url = backend_url.rstrip('/') + '/api/experiments'\n"
        "            if requests is not None:\n"
        "                r = requests.post(url, json=payload, timeout=5)\n"
        "                try:\n"
        "                    print('save_confusion_to_db:', r.status_code, r.json())\n"
        "                except Exception:\n"
        "                    print('save_confusion_to_db: response', r.status_code)\n"
        "            else:\n"
        "                # fallback: use urllib (best-effort; no TLS verification changes)\n"
        "                from urllib import request as _ur, parse as _parse\n"
        "                data = json.dumps(payload).encode('utf-8')\n"
        "                req = _ur.Request(url, data=data, headers={'Content-Type': 'application/json'})\n"
        "                try:\n"
        "                    with _ur.urlopen(req, timeout=5) as resp:\n"
        "                        print('save_confusion_to_db: status', resp.status)\n"
        "                except Exception as _e:\n"
        "                    print('save_confusion_to_db failed (urllib):', _e)\n"
        "        except Exception as _e:\n"
        "            # non-fatal helper failure\n"
        "            print('save_confusion_to_db helper error:', _e)\n"
        "\n"
        "except Exception:\n"
